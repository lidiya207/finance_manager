...
<div class="min-h-screen bg-slate-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 fade-in-up">
            ...
            <p class="text-slate-600 dark:text-slate-400 text-lg">Welcome back! Here's your financial overview</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <!-- TOTAL INCOME CARD -->
            <div class="stat-card rounded-3xl p-8 fade-in-up delay-100 border-t-4 border-emerald-500 dark:border-emerald-600">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <p class="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Total Income</p>
                        <div class="flex items-center">
                            <span class="status-indicator bg-emerald-500 mr-2"></span>
                        ...
                        @foreach($incomeByCurrency as $currency => $amount)
                            <div class="bg-white/50 rounded-2xl p-4 backdrop-blur-sm">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="currency-badge bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800">{{ $currency }}</span>
                                    <span class="text-3xl font-bold text-slate-900 dark:text-white counter-number">{{ number_format($amount, 2) }}</span>
                                </div>
                ...
                <a href="{{ route('income.create') }}"
                   class="action-btn block w-full bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 dark:hover:bg-slate-600 text-white py-4 rounded-xl text-center font-semibold shadow-md hover:shadow-lg transition-all">
            ...
            <!-- TOTAL EXPENSE CARD -->
            <div class="stat-card rounded-3xl p-8 fade-in-up delay-200 border-t-4 border-red-500 dark:border-red-600">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <div class="flex items-center">
                            ...
                            <span class="text-xs text-slate-500 dark:text-slate-400">Tracking</span>
                        </div>
                    </div>
                    <div class="icon-pulse">
                ...
                @if($expensesByCurrency->count() > 0)
                    <div class="space-y-4 mb-6">
                        @foreach($expensesByCurrency as $currency => $amount)
                            <div class="bg-white/50 rounded-2xl p-4 backdrop-blur-sm">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="currency-badge bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-100 dark:border-red-800">{{ $currency }}</span>
                                    <span class="text-3xl font-bold text-slate-900 dark:text-white counter-number">{{ number_format($amount, 2) }}</span>
                                </div>
                                <div class="progress-bar mt-3">
                ...
                <a href="{{ route('expenses.create') }}"
                   class="action-btn block w-full bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-600 hover:border-slate-900 dark:hover:border-slate-400 text-slate-900 dark:text-white py-4 rounded-xl text-center font-semibold transition-all">
                    <span class="flex items-center justify-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                        </svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 fade-in-up delay-400">
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">Financial Health</h3>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-slate-600 dark:text-slate-400">Overall Status</span>
                        <span class="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Good</span>
                    </div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: 70%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);"></div>
                    </div>
                    <p class="text-xs text-slate-500 dark:text-slate-500 mt-2">Your financial health is looking good. Keep tracking your expenses!</p>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">Quick Stats</h3>
