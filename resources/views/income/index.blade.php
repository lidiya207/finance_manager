...
@section('title', 'Income Management - Finance Manager')
...
>Income</h1>
    ...
    <div class="mt-12 flex justify-center gap-4" style="margin-top: 2rem; margin-bottom: 4rem;">
        <!-- Add Income Button -->
        <a href="{{ route('income.create') }}" class="inline-flex items-center gap-2 font-semibold px-6 py-3 rounded-2xl transition-colors duration-200 shadow-md hover:shadow-lg no-underline" style="background-color: #03205c !important; color: white !important;">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color: rgb(253, 249, 249) !important;">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
    <!-- Birr (ETB) -->
    <div class="bg-green rounded-xl shadow-md p-6 border border-gray-100 flex items-center justify-between">
        <div>
            <p class="text-sm text-gray-500">Income (Birr - ETB)</p>
    ...
    <!-- USD -->
    <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100 flex items-center justify-between" style="margin-top: 1rem; margin-bottom: 4rem;">
        <div>
            <p class="text-sm text-gray-500">Income (USD)</p>
            <p class="text-3xl md:text-4xl font-bold text-gray-900">
...
<!-- Filters + Search -->
<form method="GET" action="{{ route('income.index') }}" class="flex flex-wrap gap-4 mb-30 items-center justify-between" style="margin-top: 1rem; margin-bottom: 4rem;">
    <div class="flex gap-4 text-sm">
        <a href="{{ route('income.index') }}"
           class="px-4 py-2 rounded-full border border-gray-300 hover:bg-gray-100 {{ request()->currency ? '' : 'bg-gray-200 font-semibold' }}">
           All Time
        </a>
        <a href="{{ route('income.index', ['currency' => 'ETB']) }}"
           class="px-4 py-2 rounded-full border border-gray-300 hover:bg-green-100 {{ request('currency') == 'ETB' ? 'bg-green-200 font-semibold' : '' }}">
           ETB
        </a>
        <a href="{{ route('income.index', ['currency' => 'USD']) }}"
           class="px-4 py-2 rounded-full border border-gray-300 hover:bg-blue-100 {{ request('currency') == 'USD' ? 'bg-blue-200 font-semibold' : '' }}">
           USD
        </a>
    </div>
    <input type="text" name="search" value="{{ request('search') }}"
           placeholder="Search by source, bank, or currency..."
                ...
                <div class="flex items-center gap-4 flex-1">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-gray-500">
                            {{ \Carbon\Carbon::parse($income->occurred_at)->format('M d, Y • g:i A') }}
                        </p>
                        @if($income->bank)
...
<div class="bg-white rounded-xl shadow-md border border-gray-200 p-6 mt-16">
    <h3 class="text-xl md:text-2xl font-extrabold text-gray-900 mb-4">Income Overview (Monthly)</h3>
