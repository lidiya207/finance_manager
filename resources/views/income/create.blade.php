...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Back Link -->
    <div class="mb-6">
        <a href="{{ route('income.index') }}" class="inline-flex items-center text-gray-600 hover:text-gray-900 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        ...
        <div class="mb-8 items-center">
            ...
            <p class="text-gray-600 text-sm">Record a new income transaction</p>
        </div>

        <form method="POST" action="{{ route('income.store') }}" class="space-y-6" id="incomeForm">
            @csrf
            ...
            <!-- Income Source -->
            <div>
                <label for="source_name" class="block text-sm font-medium text-gray-900 mb-2">
                    Income Source <span class="text-red-500">*</span>
                </label>
                <input type="text" 
                       id="source_name" 
                       name="source_name" 
                       ...
                       class="block w-full px-4 py-3 text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all @error('source_id') border-red-500 @enderror">
                ...
                @error('source_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-2 text-xs text-gray-500">Type a new source name to create it automatically</p>
            </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
                <a href="{{ route('income.index') }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors font-medium">
                    Cancel
                </a>
                <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 font-semibold rounded-lg transition-colors duration-200 shadow-md hover:shadow-lg border-0" style="color: white !important; background-color: #2563eb !important;">
                    <span style="color: white !important;">Save Income</span>
                </button>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    sourceInput.addEventListener('input', function() {
        const selectedOption = Array.from(sourceList.options).find(
            option => option.value === this.value
        );
        if (selectedOption) {
            sourceIdInput.value = selectedOption.getAttribute('data-id');
        } else {
            sourceIdInput.value = '';
        }
    });
