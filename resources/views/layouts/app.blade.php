...
<body class="bg-gray-50 dark:bg-slate-900 font-sans antialiased transition-colors duration-200">
    <div id="app" class="flex flex-col h-screen overflow-hidden">
        @auth
        <!-- Top Navbar -->
        <nav class="bg-white dark:bg-slate-800 shadow-md border-b border-red-200 dark:border-slate-700 h-16 flex items-center justify-between px-6 transition-colors duration-200">
            <a href="{{ route('dashboard') }}" class="text-3xl font-extrabold tracking-widest text-red-800 dark:text-red-500 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                <span class="text-4xl">💰</span> Finance Manager
            </a>
            <div class="flex items-center">
                <span class="font-medium text-red-800 dark:text-gray-200 mr-12">{{ Auth::user()->name }}</span>
        ...
        <div class="flex flex-1 overflow-hidden">
            <!-- Sidebar -->
            <aside class="w-64 bg-gray-50 dark:bg-slate-900 border-r border-gray-200 dark:border-slate-700 transition-colors duration-200">
                <div class="flex flex-col h-full">
                    ...
                    <nav class="flex-1 px-4 py-6 overflow-y-auto">
                        ...
                        <a href="{{ route('dashboard') }}" class="flex items-center px-3 py-2.5 mb-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors {{ request()->routeIs('dashboard') ? 'bg-gray-100 dark:bg-slate-800 text-gray-900 dark:text-white' : '' }}">
            ...
            <div class="flex-1 flex flex-col overflow-hidden min-w-0">
                <main class="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-900 min-w-0 pt-20 transition-colors duration-200">
                    @yield('content')
                </main>
